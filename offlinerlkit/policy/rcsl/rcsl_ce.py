...
class RcslCEPolicy(BasePolicy):
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        prob_actions1 = self.rcsl.forward(obss, rtgs)
        # prob_actions1 = torch.clip(prob_actions1, 0.000001, 1-0.000001)
        prob_actions1 = soft_clamp(prob_actions1, 0.4, 1-0.4)
        print(prob_actions1)
        # Average over batch and dim, sum over ensembles.
        # loss = torch.pow(pred_actions - actions.to(pred_actions.device), 2).mean() # MSE error
        assert prob_actions1.shape[-1] == 1
        prob_actions0 = 1 - prob_actions1

        actions = torch.as_tensor(actions, dtype = torch.float32, device =self.device)
        loss = - actions * torch.log(prob_actions0) - (1-actions) * torch.log(prob_actions1)
        loss = loss.mean()
