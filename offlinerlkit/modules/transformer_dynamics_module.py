...
class TransformerDynamicsModel(nn.Module):
    def __init__(
        self, obs_dim, act_dim, obs_min, obs_max, act_min, act_max, ckpt_dir: str, device="cpu",
        ...
        block_size = (
            self._obs_dim + self._act_dim + self._obs_dim - 1
        )  # -1 since only need n-1 autoregressive steps
        vocab_size = 500
        # n_layer = 4
        # n_layer = 2
        # n_head = 4
        ...
        conf = GPTConfig(
            ...
            discrete_dim=self._obs_dim + self._act_dim + self._obs_dim,
            n_layer=n_layer,
            n_head=n_head,
    ...
    def fit(self, obs_act, r_next_obs, weight):
        ...
        logits, loss_p = self._model(input_discrete, targets=next_obs_discrete)
        return loss_p
