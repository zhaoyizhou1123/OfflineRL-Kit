...
from offlinerlkit.modules import ActorProb, Critic, TanhDiagGaussian, TransformerDynamicsModel, RewardDynamicsModel
from offlinerlkit.dynamics import BaseDynamics, EnsembleDynamics, TransformerDynamics
...
def get_args():
    ...
    # general
    parser.add_argument("--algo-name", type=str, default="test_dyn_trans")
...
def train(args=get_args()):
    ...
    # log
    log_dirs = make_log_dirs(args.task, args.algo_name, args.seed, vars(args), part = "dynamics_trans", record_params=['n_layer', 'sample_ratio'])
    ...
    reward_model = RewardDynamicsModel(
        obs_dim = obs_dim,
        act_dim = action_dim,
        r_hidden_dims=args.dynamics_hidden_dims,
        device = args.device
    ...
    diff_log_dirs = make_log_dirs(args.task, args.algo_name, args.seed, vars(args), part="diffusion", record_params=['n_layer', 'sample_ratio'])
    # key: output file name, value: output handler type
    diff_output_config = {
        "consoleout_backup": "stdout",
        "policy_training_progress": "csv",
    ...
    #             if pred_ret >= 1:
    #                 pred_cnt += 1
    #             print(f"Epoch {epoch}, true total return {true_ret}, predicted {pred_ret}")
    ...
    rollout_save_dir = make_log_dirs(args.task, args.algo_name, args.seed, vars(args), part="rollout", record_params=['n_layer', 'sample_ratio'])
