...
from offlinerlkit.policy import DiffusionBC, AutoregressiveCategoricalPolicy
...
def get_args():
    ...
    # general
    parser.add_argument("--algo-name", type=str, default="regress_ctg")
    ...
    parser.add_argument("--output_bins", type = int, default = 128, help="number of action categories")
...
def train(args=get_args()):
    ...
    if args.task == 'pickplace' or args.task == 'pickplace_easy':
        ...
        print(f"Action min: {action_min}, action max {action_max}")
    ...
    rcsl_policy = AutoregressiveCategoricalPolicy(
        obs_dim=obs_dim,
        act_dim = action_dim,
        feature_dim = 1,
        output_min = action_min,
        output_max = action_max,
        output_bins = args.output_bins,
        device = args.device
    )
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(rcsl_policy.optimizer, args.rcsl_epoch)
    ...
    task_name = args.task
    rcsl_log_dirs = make_log_dirs(task_name, args.algo_name, args.seed, vars(args), part='rcsl_regress', record_params=['eval_episodes', 'output_bins'])
    # key: output file name, value: output handler type
    rcsl_output_config = {
        "consoleout_backup": "stdout",
        "policy_training_progress": "csv",
